...
platform: devto
status: live
published_url: https://dev.to/sai_ram_0000/trelix-v2110-to-v311-six-feature-areas-every-one-of-them-off-by-default-1go1
published_date: 2026-08-15
devto_id: 4404473
tags: ["python", "security", "systemdesign", "opensource"]
cover_image: assets/trelix-v3-1-1-release/cover.png
canonical_url: https://anvilry.vercel.app/notes/trelix-v3-1-1-release
note: |
  Covers the v2.11.0 -> v3.1.1 span (six releases). v3.1.1 is cut and dated 2026-08-15 but NOT yet
  tagged or on PyPI at time of writing (PyPI latest = 3.1.0) - the body is written to avoid any
  install-it-now claim.

  Published via `inkforge publish --platform devto` (no --published flag) as a draft on
  2026-08-15, then flipped live by the author the same day. Verified against the Dev.to API at
  both stages: canonical_url set to the Anvilry URL, tags resolved to
  python/security/systemdesign/opensource, body byte-identical to the primary article except
  Dev.to auto-tagging the first code fence as ```console.

  NOTE ON THE URL: publishing REPLACED the draft's `-26gf-temp-slug-2646138` suffix with the
  permanent `-1go1`. Same article id (4404473) throughout - no duplicate post was created, and
  the account has no leftover trelix draft. That differs from the v2.11.0 round, where a rewrite
  did leave a stale second draft behind.

  Tags note: the CLI reads the PRIMARY article's frontmatter, not this file - it lowercases,
  strips hyphens and takes the first 4. The primary tag order was set so the best four win.
  Editing `tags` in this file alone does not change what Dev.to receives.
---
