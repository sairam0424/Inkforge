...
platform: devto
status: live
published_url: https://dev.to/sai_ram_0000/tracehub-mcp-giving-ai-assistants-a-real-query-interface-into-your-llm-traces-22ck
published_date: 2026-09-13
devto_id: 4643697
tags: ["ai", "opensource", "observability", "python"]
cover_image: assets/how-i-built-tracehub-mcp/cover.png
canonical_url: https://anvilry.vercel.app/notes/how-i-built-tracehub-mcp
note: |
  LAUNCH/introduction article for tracehub-mcp (org mcpsmiths, forked from
  traceloop/opentelemetry-mcp-server), current shipping version v0.3.0, live on PyPI.
  Dev.to body is byte-identical to the primary article - the adapt agent correctly
  determined no changes were needed (no H1, no canonical line, no forbidden elements in
  the primary draft already). Independently re-verified ~30 load-bearing claims by hand
  against the live repo post-workflow - exact regex patterns, exact test file line counts
  (781/1117), exact commit titles for the CI-hardening and version-drift fixes, and the
  README/NOTICE quotes used near-verbatim. Re-ran the test suite myself: 467 passed, 2
  skipped, 87% coverage (matches the article; the README's own "458" figure is stale and
  the article explicitly corrects it once, by name). Re-confirmed 5 open Dependabot PRs
  live on GitHub at write time. Zero discrepancies found - the second consecutive
  zero-discrepancy verification pass in this content tree.
---
